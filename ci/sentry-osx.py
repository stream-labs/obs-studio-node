...
for r, d, f in os.walk(server_path):
    for file in f:
        if '.so' in file or not '.' in file:
            ...
            os.system("sentry-cli --auth-token ${SENTRY_AUTH_TOKEN} upload-dif --org streamlabs-obs --project obs-server " +
...
for r, d, f in os.walk(obs_plugins_path):
    for file in f:
        if '.so' in file or not '.' in file:
            ...
            os.system("sentry-cli --auth-token ${SENTRY_AUTH_TOKEN} upload-dif --org streamlabs-obs --project obs-server " +
            obs_plugins_path + file + ".dSYM/Contents/Resources/DWARF/" + file)