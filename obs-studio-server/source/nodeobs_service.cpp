...
bool OBS_service::resetAudioContext(void)
{
	...
	ai.samples_per_sec = 
		uint32_t(config_get_uint(ConfigManager::getInstance().getBasic(), "Audio", "SampleRate"));
...
void OBS_service::associateAudioAndVideoToTheCurrentStreamingContext(void)
{
	...
	if (strcmp(advancedMode, "Advanced") == 0) {
		...
		if (rescale && rescaleRes && *rescaleRes) {
			if (sscanf_s(rescaleRes, "%ux%u", &cx, &cy) != 2) {
				cx = 0;
				cy = 0;
			}
...
void OBS_service::UpdateFFmpegOutput(void)
{
	...
	if (rescale && rescaleRes && *rescaleRes) {
		int width;
		int height;
		int val = sscanf_s(rescaleRes, "%dx%d", &width, &height);
