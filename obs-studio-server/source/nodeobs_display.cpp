...
void OBS::Display::SystemWorker() {
	...
	do {
		...
		case SystemWorkerMessage::CreateWindow:
		{
			...
			// WS_EX_LAYERED and WS_EX_TRANSPARENT are required for NCHITTEST to work across processes.
			DWORD windowStyle = WS_EX_LAYERED | WS_EX_TRANSPARENT;

			HWND newWindow = CreateWindowEx(
				windowStyle,
				((LPCWSTR)DisplayWndClassAtom), TEXT("SlobsChildWindowPreview"),
				WS_VISIBLE | WS_POPUP,
			...
			if (!newWindow) {
				...
				SetWindowLongPtrW(newWindow, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(this));
				SetWindowPos(newWindow, 0, 0, 0, 0, 0, SWP_NOZORDER | SWP_NOACTIVATE | SWP_NOMOVE | SWP_NOSIZE);
