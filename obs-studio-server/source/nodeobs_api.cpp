...
/* FIXME DEPRECATED This is the primary source of confusing bugs more than likely. */
config_t* OBS_API::openConfigFile(std::string configFile)
{
	...
	int result = config_open(&config, configFile.c_str(), CONFIG_OPEN_ALWAYS);
	...
	* than 1280x720 */
	while ((scale_cx * scale_cy) > (1280 * 720) &&
	       scaled_vals[i] > 0.0
	) {
