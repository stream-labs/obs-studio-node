...
void OBS_API::destroyOBS_API(void)
{
	...
	if (osn::Source::Manager::GetInstance().size() > 0 || osn::Scene::Manager::GetInstance().size() > 0 ||
		...
		obs_wait_for_destroy_queue();
		blog(LOG_WARNING, "OBS_API::destroyOBS_API - obs_wait_for_destroy_queue has finished, osn::Source::Manager::GetInstance() size is %d", osn::Source::Manager::GetInstance().size());
