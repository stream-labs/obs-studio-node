...
void osn::AudioEncoder::Register(ipc::server& srv)
{
	...
	cls->register_function(std::make_shared<ipc::function>(
	    "GetBitrate", std::vector<ipc::type>{ipc::type::UInt64}, GetBitrate));
	cls->register_function(std::make_shared<ipc::function>(
	    "SetBitrate", std::vector<ipc::type>{ipc::type::UInt64, ipc::type::UInt32}, SetBitrate));

	srv.register_collection(cls);
}

void osn::AudioEncoder::Create(
    void*                          data,
    const int64_t                  id,
    ...
    std::vector<ipc::value>&       rval)
{
	obs_encoder_t* audioEncoder = 
		obs_audio_encoder_create("ffmpeg_aac", "audio", nullptr, 0, nullptr);
...
{
	obs_encoder_t* audioEncoder =
		osn::AudioEncoder::Manager::GetInstance().find(args[0].value_union.ui64);
	if (!audioEncoder) {
...
void osn::AudioEncoder::GetBitrate(
    void*                          data,
    const int64_t                  id,
    const std::vector<ipc::value>& args,
    std::vector<ipc::value>&       rval)
...
void osn::AudioEncoder::SetBitrate(
    void*                          data,
    const int64_t                  id,
    const std::vector<ipc::value>& args,
    std::vector<ipc::value>&       rval)
