    ...
    Napi::Function func =
        ...
        "AudioTrack",
        {
            ...
            InstanceAccessor("bitrate",
                &osn::AudioTrack::GetBitrate,
                &osn::AudioTrack::SetBitrate),
...
osn::AudioTrack::AudioTrack(const Napi::CallbackInfo& info)
    : Napi::ObjectWrap<osn::AudioTrack>(info) {
...
Napi::Value osn::AudioTrack::Create(const Napi::CallbackInfo& info) {
    ...
    std::vector<ipc::value> response =
        conn->call_synchronous_helper("AudioTrack", "Create", {});

    if (!ValidateResponse(info, response))
        return info.Env().Undefined();

    auto instance =
        osn::AudioTrack::constructor.New({
...
void osn::AudioTrack::SetAtIndex(const Napi::CallbackInfo& info) {
    osn::AudioTrack* audioTrack =
        Napi::ObjectWrap<osn::AudioTrack>::Unwrap(info[0].ToObject());
    uint32_t index = info[1].ToNumber().Uint32Value();

    if (!audioTrack) {
        Napi::TypeError::New(
            info.Env(),
            "Invalid audioTrack argument").ThrowAsJavaScriptException();
        return;
    }
    ...
    conn->call(
        "AudioTrack",
        "SetAtIndex",
        {ipc::value(audioTrack->uid), ipc::value(index)});
}
...
void osn::AudioTrack::SetBitrate(
