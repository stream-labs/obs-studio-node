...
#define ASSERT_GET_OBJECT_FIELD(object, field, var)                                           \
	if (!utilv8::GetFromObject((object), (field), (var))) {                                   \
		Napi::Error::New(info.Env(), FIELD_NAME(info, std::string(__FUNCTION_NAME__)          \
		+ ": Unexpected type."));                                                             \
		return;                                                                               \
	}
...
namespace utilv8
{
	...
	template<typename T>
	inline Napi::Value ToValue(const Napi::CallbackInfo& info, std::vector<T> v)
	{
	...
	template<typename T>
	inline Napi::Value ToValue(const Napi::CallbackInfo& info, std::list<T> v)
	{
		// auto rv = v8::Array::New(v8::Isolate::GetCurrent());
		auto rv = Napi::Array::New(info.Env(), v.size());
		for (size_t idx = 0; idx < v.size(); idx++) {
	...
	inline Napi::Value ToValue(const Napi::CallbackInfo& info, std::map<A, B> v)
	{
	...
	template<typename T>
	inline Napi::Value ToValue(const Napi::CallbackInfo& info, std::map<uint32_t, T> v)
	{
		// auto rv = v8::Object::New(v8::Isolate::GetCurrent());
		auto rv = Napi::Object::New(info.Env());
		for (auto kv : v) {
	...
	// inline bool FromValue(Napi::Value l, char*& r)
	// {
	// 	if (l.IsString()) {
	// 		// auto                  v8s = l->ToString();
	// 		// v8::String::Utf8Value utfv8(v8s);
