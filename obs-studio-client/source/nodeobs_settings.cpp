		...
		for (int j = 0; j < params.size(); j++) {
			...
			if (params.at(j).currentValue.size() > 0) {
				if (params.at(j).type.compare("OBS_PROPERTY_LIST") == 0
					...
					parameter->Set(
					    v8::String::NewFromUtf8(isolate, "currentValue"),
					...
					parameter->Set(v8::String::NewFromUtf8(isolate, "currentValue"), v8::Integer::New(isolate, *value));
					...
					parameter->Set(
					    v8::String::NewFromUtf8(isolate, "currentValue"), v8::Boolean::New(isolate, (*value)));
				} else if (params.at(j).type.compare("OBS_PROPERTY_DOUBLE") == 0) {
					double* value = reinterpret_cast<double*>(params.at(j).currentValue.data());
					parameter->Set(v8::String::NewFromUtf8(isolate, "currentValue"), v8::Number::New(isolate, *value));
				}
			} else {
