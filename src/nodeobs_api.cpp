...
void OBS_API::OBS_API_initAPI(const FunctionCallbackInfo<Value> &args)
{
	...
	SetEnvironmentVariable("OBS_DATA_PATH",
	                       std::string(g_moduleDirectory + "/libobs/data").c_str());

	std::vector<char> userData = std::vector<char>(1024);
	os_get_config_path(userData.data(), userData.capacity() - 1,
	                   "slobs-client/plugin_config");
	...
	if(isOBS_installedValue) {
		v8::String::Utf8Value firstProfile(getOBS_existingProfiles()->Get(
		                                         0)->ToString());
		OBS_currentProfile = std::string(*firstProfile);

		v8::String::Utf8Value firstSceneCollection(
		      getOBS_existingSceneCollections()->Get(0)->ToString());
...
void OBS_API::OBS_API_getPathConfigDirectory(const FunctionCallbackInfo<Value>
                                             &args)
{
	Isolate *isolate = args.GetIsolate();

	args.GetReturnValue().Set(String::NewFromUtf8(isolate,
	                                              getPathConfigDirectory().c_str()));
}

void OBS_API::OBS_API_setPathConfigDirectory(const FunctionCallbackInfo<Value>
                                             &args)
{
	Isolate *isolate = args.GetIsolate();
...
void OBS_API::OBS_API_getOBS_existingProfiles(const
                                              FunctionCallbackInfo<Value> &args)
{
	args.GetReturnValue().Set(getOBS_existingProfiles());
}

void OBS_API::OBS_API_getOBS_existingSceneCollections(const
                                                      FunctionCallbackInfo<Value> &args)
{
	args.GetReturnValue().Set(getOBS_existingSceneCollections());
}

void OBS_API::OBS_API_getOBS_currentProfile(const FunctionCallbackInfo<Value>
                                            &args)
{
	Isolate *isolate = args.GetIsolate();

	args.GetReturnValue().Set(String::NewFromUtf8(isolate,
	                                              getOBS_currentProfile().c_str()));
}

void OBS_API::OBS_API_setOBS_currentProfile(const FunctionCallbackInfo<Value>
                                            &args)
{
	Isolate *isolate = args.GetIsolate();
...
void OBS_API::OBS_API_getOBS_currentSceneCollection(const
                                                    FunctionCallbackInfo<Value> &args)
{
	Isolate *isolate = args.GetIsolate();

	args.GetReturnValue().Set(String::NewFromUtf8(isolate,
	                                              getOBS_currentSceneCollection().c_str()));
}

void OBS_API::OBS_API_setOBS_currentSceneCollection(const
                                                    FunctionCallbackInfo<Value> &args)
{
	Isolate *isolate = args.GetIsolate();
...
void OBS_API::openAllModules(void)
{
	...
	for (int i = 0; i < num_paths; ++i) {
		...
		* shared library. */
		...
		for (os_dirent *ent = os_readdir(plugin_dir); ent != nullptr;
		            ent = os_readdir(plugin_dir)) {
			...
			obs_module_t *module;
			int result = obs_open_module(&module, plugin_path.c_str(),
			                             plugin_data_path.c_str());

			switch (result) {
			case MODULE_SUCCESS:
				break;
			case MODULE_FILE_NOT_FOUND:
				std::cerr << "Unable to load '" << plugin_path << "', could not find file." <<
				          std::endl;
				continue;
			case MODULE_MISSING_EXPORTS:
				std::cerr << "Unable to load '" << plugin_path << "', missing exports." <<
				          std::endl;
				continue;
			case MODULE_INCOMPATIBLE_VER:
				std::cerr << "Unable to load '" << plugin_path << "', incompatible version." <<
				          std::endl;
				continue;
			case MODULE_ERROR:
				std::cerr << "Unable to load '" << plugin_path << "', generic error." <<
				          std::endl;
				continue;
			default:
				continue;
...
Local<Object> OBS_API::getPerformanceStatistics(void)
{
	...
	statistics->Set(String::NewFromUtf8(isolate, "CPU"), Number::New(isolate,
	                                                                 getCPU_Percentage()));
	statistics->Set(String::NewFromUtf8(isolate, "numberDroppedFrames"),
	                Number::New(isolate, getNumberOfDroppedFrames()));
	statistics->Set(String::NewFromUtf8(isolate, "percentageDroppedFrames"),
	                Number::New(isolate, getDroppedFramesPercentage()));
	statistics->Set(String::NewFromUtf8(isolate, "bandwidth"), Number::New(isolate,
	                                                                       getCurrentBandwidth()));
	statistics->Set(String::NewFromUtf8(isolate, "frameRate"), Number::New(isolate,
	                                                                       getCurrentFrameRate()));
...
std::vector<std::string> exploreDirectory(std::string directory,
                                          std::string typeToReturn)
{
...
Local<Array> OBS_API::getOBS_existingProfiles(void)
{
	...
	std::vector<std::string> vectorExistingProfiles = exploreDirectory(pathProfiles,
	                                                                   "directories");

	for(int i=0; i<vectorExistingProfiles.size(); i++)
		listExistingProfiles->Set(i, String::NewFromUtf8(isolate,
		                                                 vectorExistingProfiles.at(i).c_str()));
...
Local<Array> OBS_API::getOBS_existingSceneCollections(void)
{
	...
	std::vector<std::string> files = exploreDirectory(pathSceneCollections,
	                                                  "files");
	...
	for(int i=0; i<files.size(); i++) {
		if(files.at(i).substr(files.at(i).find_last_of(".") + 1) == "json") {
			...
			listExistingSceneCollections->Set(indexArray++, String::NewFromUtf8(isolate,
			                                                                    files.at(i).c_str()));
		}
	}
                                     ...
                                     LPARAM   dwData)
{
	...
	if (GetMonitorInfo(hMonitor, &info)) {
		std::vector<Screen> *resolutions = reinterpret_cast<std::vector<Screen>*>
		                                   (dwData);
...
config_t *OBS_API::openConfigFile(std::string configFile)
{
	...
	std::string subString = configFile.substr(configFile.size() - basic.size(),
	                                          basic.size()).c_str();
