...
void OBS_settings::OBS_settings_getListCategories(const
                                                  FunctionCallbackInfo<Value> &args)
{
	args.GetReturnValue().Set(getListCategories());
}

void OBS_settings::OBS_settings_getSettings(const FunctionCallbackInfo<Value>
                                            &args)
{
...
void OBS_settings::OBS_settings_saveSettings(const FunctionCallbackInfo<Value>
                                             &args)
{
	Isolate *isolate = args.GetIsolate();
                                                  ...
                                                  config_t *config, std::string section, bool isVisible, bool isEnabled)
{
	...
	for(int i=0; i<entries.size(); i++) {
		...
		if(!currentValue.empty() && currentValue.compare("currentValue") == 0 ) {
			parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
			               String::NewFromUtf8(isolate, entries.at(i).at(3).second.c_str()));
			...
			if(type.compare("OBS_PROPERTY_LIST") == 0 ||
				...
				const char *currentValue = config_get_string(config, section.c_str(),
				                                             name.c_str());
				...
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               Integer::New(isolate, config_get_int(config, section.c_str(), name.c_str())));

			else if(type.compare("OBS_PROPERTY_UINT") == 0)
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               Integer::New(isolate, config_get_uint(config, section.c_str(), name.c_str())));

			else if(type.compare("OBS_PROPERTY_BOOL") == 0)
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               Boolean::New(isolate, config_get_bool(config, section.c_str(), name.c_str())));

			else if(type.compare("OBS_PROPERTY_DOUBLE") == 0)
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               Integer::New(isolate, config_get_double(config, section.c_str(),
				                                                       name.c_str())));
		}
		if(entries.at(i).size() > 3) {
		...
		parameter->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
		                                                                     isVisible));
		parameter->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
		                                                                     isEnabled));
		parameter->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
		                                                                    false));

		subCategoryParameters->Set(i, parameter);
	}
	subCategory->Set(String::NewFromUtf8(isolate, "nameSubCategory"),
	                 String::NewFromUtf8(isolate, nameSubCategory.c_str()));
	subCategory->Set(String::NewFromUtf8(isolate, "parameters"),
...
Local<Array> OBS_settings::getGeneralSettings()
{
	...
	config_t *config;
	int result = config_open(&config, globalConfigFile.c_str(),
	...
	// General
	generalSettings->Set(0, serializeSettingsData("General", entries, config,
	                                              "BasicWindow", true, true));
	entries.clear();

	// Output
	std::vector<std::pair<std::string, std::string>> warnBeforeStartingStream;
	warnBeforeStartingStream.push_back(std::make_pair("name",
	                                                  "WarnBeforeStartingStream"));
	warnBeforeStartingStream.push_back(std::make_pair("type", "OBS_PROPERTY_BOOL"));
	warnBeforeStartingStream.push_back(std::make_pair("description",
	...
	warnBeforeStoppingStream.push_back(std::make_pair("name",
	                                                  "WarnBeforeStoppingStream"));
	warnBeforeStoppingStream.push_back(std::make_pair("type", "OBS_PROPERTY_BOOL"));
	warnBeforeStoppingStream.push_back(std::make_pair("description",
	...
	recordWhenStreaming.push_back(std::make_pair("description",
	...
	keepRecordingWhenStreamStops.push_back(std::make_pair("name",
	                                                      "KeepRecordingWhenStreamStops"));
	keepRecordingWhenStreamStops.push_back(std::make_pair("type",
	                                                      "OBS_PROPERTY_BOOL"));
	keepRecordingWhenStreamStops.push_back(std::make_pair("description",
	...
	replayBufferWhileStreaming.push_back(std::make_pair("name",
	                                                    "ReplayBufferWhileStreaming"));
	replayBufferWhileStreaming.push_back(std::make_pair("type",
	                                                    "OBS_PROPERTY_BOOL"));
	replayBufferWhileStreaming.push_back(std::make_pair("description",
	...
	keepReplayBufferStreamStops.push_back(std::make_pair("name",
	                                                     "KeepReplayBufferStreamStops"));
	keepReplayBufferStreamStops.push_back(std::make_pair("type",
	                                                     "OBS_PROPERTY_BOOL"));
	keepReplayBufferStreamStops.push_back(std::make_pair("description",
	                                                     "Keep replay buffer active when stream stops"));
	entries.push_back(keepReplayBufferStreamStops);

	generalSettings->Set(1, serializeSettingsData("Output", entries, config,
	                                              "BasicWindow", true, true));
	entries.clear();
	...
	screenSnapping.push_back(std::make_pair("description",
	...
	sourceSnapping.push_back(std::make_pair("description",
	...
	centerSnapping.push_back(std::make_pair("description",
	                                        "Snap Sources to horizontal and vertical center"));
	entries.push_back(centerSnapping);

	generalSettings->Set(2, serializeSettingsData("Source Alignement Snapping",
	                                              entries, config, "BasicWindow", true, true));
	entries.clear();
	...
	hideProjectorCursor.push_back(std::make_pair("description",
	...
	projectorAlwaysOnTop.push_back(std::make_pair("description",
	...
	saveProjectors.push_back(std::make_pair("description",
	                                        "Save projectors on exit"));
	entries.push_back(saveProjectors);

	generalSettings->Set(3, serializeSettingsData("Projectors", entries, config,
	                                              "BasicWindow", true, true));
	entries.clear();
	...
	sysTrayWhenStarted.push_back(std::make_pair("description",
	...
	sysTrayMinimizeToTray.push_back(std::make_pair("name",
	                                               "SysTrayMinimizeToTray"));
	sysTrayMinimizeToTray.push_back(std::make_pair("type", "OBS_PROPERTY_BOOL"));
	sysTrayMinimizeToTray.push_back(std::make_pair("description",
	                                               "Always minimize to system tray instead of task bar"));
	entries.push_back(sysTrayMinimizeToTray);

	generalSettings->Set(4, serializeSettingsData("System Tray", entries, config,
	                                              "BasicWindow", true, true));
	entries.clear();
...
void OBS_settings::saveGeneralSettings(Local<Array> generalSettings,
                                       std::string pathConfigDirectory)
{
	...
	int result = config_open(&config, pathConfigDirectory.c_str(),
	                         CONFIG_OPEN_EXISTING);
	...
	for(int i=0; i<generalSettings->Length(); i++) {
		Local<Object> generalSettingsObject = v8::Local<v8::Object>::Cast(
		                                            generalSettings->Get(i));

		v8::String::Utf8Value param0(generalSettingsObject->Get(String::NewFromUtf8(
		                                                              isolate, "nameSubCategory")));
		std::string nameSubcategory = std::string(*param0);

		Local<Array> parameters = v8::Local<v8::Array>::Cast(generalSettingsObject->Get(
		                                                           String::NewFromUtf8(isolate, "parameters")));
		...
		for(int j=0; j<parameters->Length(); j++) {
			...
			v8::String::Utf8Value param0(parameter->Get(String::NewFromUtf8(isolate,
			                                                                "name")));
			name = std::string(*param0);

			v8::String::Utf8Value param1(parameter->Get(String::NewFromUtf8(isolate,
			                                                                "type")));
			type = std::string(*param1);

			if(type.compare("OBS_PROPERTY_LIST") == 0) {
				v8::String::Utf8Value param2(parameter->Get(String::NewFromUtf8(isolate,
				                                                                "currentValue")));
				value = std::string(*param2);
				config_set_string(config, "BasicWindow", name.c_str(), value.c_str());
			} else if(type.compare("OBS_PROPERTY_INT") == 0) {
				int64_t value = parameter->Get(String::NewFromUtf8(isolate,
				                                                   "currentValue"))->NumberValue();
				config_set_int(config, "BasicWindow", name.c_str(), value);
			} else if(type.compare("OBS_PROPERTY_UINT") == 0) {
				uint64_t value = parameter->Get(String::NewFromUtf8(isolate,
				                                                    "currentValue"))->NumberValue();
				...
				double value = parameter->Get(String::NewFromUtf8(isolate,
				                                                  "currentValue"))->NumberValue();
...
Local<Array> OBS_settings::getStreamSettings()
{
	...
	streamType->Set(String::NewFromUtf8(isolate, "nameSubCategory"),
	                String::NewFromUtf8(isolate, "Untitled"));
	streamType->Set(String::NewFromUtf8(isolate, "parameters"),
	                subCategoryParametersStreamType);

	parameter->Set(String::NewFromUtf8(isolate, "name"),
	               String::NewFromUtf8(isolate, "streamType"));

	parameter->Set(String::NewFromUtf8(isolate, "type"),
	               String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	...
	while (obs_enum_service_types(index++, &type)) {
		...
		parameterValue->Set(String::NewFromUtf8(isolate,
	...
	parameter->Set(String::NewFromUtf8(isolate, "description"),
	               String::NewFromUtf8(isolate, "Stream Type"));

	const char *servType = obs_service_get_type(currentService);

	parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
	               String::NewFromUtf8(isolate, obs_service_get_type(currentService)));
	...
	while(property) {
		...
		parameter->Set(String::NewFromUtf8(isolate, "name"),
		               String::NewFromUtf8(isolate, obs_property_name(property)));
		...
		if(count == 0) {
			if(strcmp(obs_property_name(property), "key") == 0) {
				...
				if(stream_key != NULL)
					parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
					               String::NewFromUtf8(isolate, stream_key));
			...
			if(strcmp(obs_property_name(property), "server") == 0) {
				...
				if(server != NULL)
					parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
					               String::NewFromUtf8(isolate, server));
			...
			if(strcmp(obs_property_name(property), "username") == 0) {
				...
				if(username != NULL)
					parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
					               String::NewFromUtf8(isolate, username));
			...
			if(strcmp(obs_property_name(property), "use_auth") == 0) {
				...
				formatString = "OBS_PROPERTY_BOOL";
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               Integer::New(isolate, use_auth));
			}
		} else {
			if (format == OBS_COMBO_FORMAT_INT)
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               Integer::New(isolate, obs_data_get_double(settings,
				                                                         obs_property_name(property))));
			else if (format == OBS_COMBO_FORMAT_FLOAT)
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               Number::New(isolate, obs_data_get_double(settings,
				                                                        obs_property_name(property))));
			else if (format == OBS_COMBO_FORMAT_STRING) {
				currentServiceName = obs_data_get_string(settings, obs_property_name(property));
				parameter->Set(String::NewFromUtf8(isolate, "currentValue"),
				               String::NewFromUtf8(isolate, currentServiceName));
			}
			parameter->Set(String::NewFromUtf8(isolate, "values"), values);
		}
		v8::String::Utf8Value param1(parameter->Get(String::NewFromUtf8(isolate,
		...
		parameter->Set(String::NewFromUtf8(isolate, "type"),
		               String::NewFromUtf8(isolate, formatString.c_str()));

		parameter->Set(String::NewFromUtf8(isolate, "description"),
		               String::NewFromUtf8(isolate, obs_property_description(property)));

		parameter->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
		                                                                     obs_property_visible(property)));

		parameter->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
		                                                                     isCategoryEnabled));

		if(formatString.compare("OBS_PROPERTY_EDIT_TEXT") == 0
	...
	serviceConfiguration->Set(String::NewFromUtf8(isolate, "nameSubCategory"),
	                          String::NewFromUtf8(isolate, "Untitled"));
	serviceConfiguration->Set(String::NewFromUtf8(isolate, "parameters"),
	                          subCategoryParametersServiceConfiguration);
...
void OBS_settings::saveStreamSettings(Local<Array> streamSettings)
{
	...
	std::string currentServiceName = obs_data_get_string(obs_service_get_settings(
	                                                           currentService), "service");

	for(int i=0; i<streamSettings->Length(); i++) {
		Local<Object> streamSettingsCategory = v8::Local<v8::Object>::Cast(
		                                             streamSettings->Get(i));

		v8::String::Utf8Value param0(streamSettingsCategory->Get(String::NewFromUtf8(
		                                                               isolate, "nameSubCategory")));
		std::string nameSubcategory = std::string(*param0);

		Local<Array> parameters = v8::Local<v8::Array>::Cast(
		                                streamSettingsCategory->Get(String::NewFromUtf8(isolate, "parameters")));
		...
		for(int j=0; j<parameters->Length(); j++) {
			...
			if(type.compare("OBS_PROPERTY_LIST") == 0 ||
			            type.compare("OBS_PROPERTY_EDIT_TEXT") == 0) {
				v8::String::Utf8Value param2(object->Get(String::NewFromUtf8(isolate,
				                                                             "currentValue")));
				value = std::string(*param2);
				...
				if(name.compare("streamType") == 0) {
					...
					settings = obs_service_defaults(newserviceTypeValue.c_str());
				            ...
				            && value.compare(currentServiceName) != 0)
					serviceChanged = true;
				...
				int64_t value = object->Get(String::NewFromUtf8(isolate,
				                                                "currentValue"))->NumberValue();
				obs_data_set_int(settings, name.c_str(), value);
			} else if(type.compare("OBS_PROPERTY_BOOL") == 0) {
				uint64_t value = object->Get(String::NewFromUtf8(isolate,
				                                                 "currentValue"))->NumberValue();
				obs_data_set_bool(settings, name.c_str(), value);
			} else if(type.compare("OBS_PROPERTY_DOUBLE") == 0) {
				double value = object->Get(String::NewFromUtf8(isolate,
				                                               "currentValue"))->NumberValue();
	...
	if (!obs_data_save_json_safe(data, OBS_API::getServiceConfigPath().c_str(),
	                             "tmp", "bak"))
...
void OBS_settings::getAvailableEncoders(
...
void OBS_settings::getSimpleOutputSettings(Local<Array> outputSettings,
                                           config_t *config, bool isCategoryEnabled)
{
	...
	for (auto &entry : bitrateMap)
		aBitrate.push_back(std::make_pair(std::to_string(entry.first),
	...
	useAdvanced.push_back(std::make_pair("description",
	                                     "Enable Advanced Encoder Settings"));
	...
	if(config_get_bool(config, "SimpleOutput","UseAdvanced")) {
		...
		enforceBitrate.push_back(std::make_pair("description",
		                                        "Enforce streaming service bitrate limits"));
		...
		if (strcmp(encoder, SIMPLE_ENCODER_QSV) == 0) {
			...
			preset.push_back(std::make_pair("description",
			                                "Encoder Preset (higher = less CPU)"));
	...
	outputSettings->Set(1, serializeSettingsData("Streaming", entries, config,
	                                             "SimpleOutput", true, isCategoryEnabled));
	entries.clear();
	...
	fileNameWithoutSpace.push_back(std::make_pair("description",
	                                              "Generate File Name without Space"));
	...
	recQuality.push_back(
	      std::make_pair("Indistinguishable Quality, Large File Size", "HQ"));
	recQuality.push_back(
	      std::make_pair("Lossless Quality, Tremendously Large File Size", "Lossless"));
	...
	outputSettings->Set(2, serializeSettingsData("Recording", entries, config,
	                                             "SimpleOutput", true, isCategoryEnabled));
}

void OBS_settings::getEncoderSettings(Isolate *isolate,
                                      ...
                                      Local<Array> *subCategoryParameters, int index, bool isCategoryEnabled)
{
	...
	while(property) {
		...
		entryObject->Set(String::NewFromUtf8(isolate, "name"),
		                 String::NewFromUtf8(isolate, name.c_str()));

		switch(typeProperty) {
		case OBS_PROPERTY_BOOL: {
			entryObject->Set(String::NewFromUtf8(isolate, "type"),
			                 String::NewFromUtf8(isolate, "OBS_PROPERTY_BOOL"));
		...
		case OBS_PROPERTY_INT: {
			entryObject->Set(String::NewFromUtf8(isolate, "type"),
			                 String::NewFromUtf8(isolate, "OBS_PROPERTY_INT"));
			entryObject->Set(String::NewFromUtf8(isolate, "description"),
			                 String::NewFromUtf8(isolate, obs_property_description(property)));
			entryObject->Set(String::NewFromUtf8(isolate, "currentValue"),
		...
		case OBS_PROPERTY_FLOAT: {
			entryObject->Set(String::NewFromUtf8(isolate, "type"),
			                 String::NewFromUtf8(isolate, "OBS_PROPERTY_FLOAT"));
			entryObject->Set(String::NewFromUtf8(isolate, "description"),
			                 String::NewFromUtf8(isolate, obs_property_description(property)));
			entryObject->Set(String::NewFromUtf8(isolate, "currentValue"),
		...
		case OBS_PROPERTY_TEXT: {
			entryObject->Set(String::NewFromUtf8(isolate, "type"),
			                 String::NewFromUtf8(isolate, "OBS_PROPERTY_TEXT"));
			entryObject->Set(String::NewFromUtf8(isolate, "description"),
			                 String::NewFromUtf8(isolate, obs_property_description(property)));
		...
		case OBS_PROPERTY_PATH: {
			entryObject->Set(String::NewFromUtf8(isolate, "type"),
			                 String::NewFromUtf8(isolate, "OBS_PROPERTY_PATH"));
			entryObject->Set(String::NewFromUtf8(isolate, "description"),
			                 String::NewFromUtf8(isolate, obs_property_description(property)));
		...
		case OBS_PROPERTY_LIST: {
			...
			for(int i=0; i<count; i++) {
				...
				if (format == OBS_COMBO_FORMAT_INT) {
					...
					entryObject->Set(String::NewFromUtf8(isolate, "subType"),
					                 String::NewFromUtf8(isolate, "OBS_COMBO_FORMAT_INT"));
					...
					entryObject->Set(String::NewFromUtf8(isolate, "subType"),
					                 String::NewFromUtf8(isolate, "OBS_COMBO_FORMAT_FLOAT"));
					object->Set(String::NewFromUtf8(isolate, itemName.c_str()),
					            Number::New(isolate, val));
				} else if (format == OBS_COMBO_FORMAT_STRING) {
					valueString = obs_property_list_item_string(property, i);
					entryObject->Set(String::NewFromUtf8(isolate, "subType"),
					                 String::NewFromUtf8(isolate, "OBS_COMBO_FORMAT_STRING"));
					object->Set(String::NewFromUtf8(isolate, itemName.c_str()),
		...
		case OBS_PROPERTY_EDITABLE_LIST: {
			entryObject->Set(String::NewFromUtf8(isolate, "type"),
			                 String::NewFromUtf8(isolate, "OBS_PROPERTY_EDITABLE_LIST"));
			entryObject->Set(String::NewFromUtf8(isolate, "description"),
			                 String::NewFromUtf8(isolate, obs_property_description(property)));
		...
		entryObject->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
		                                                                       obs_property_visible(property)));
		...
		entryObject->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
		                                                                       isEnabled));
		entryObject->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
		                                                                      false));
                                                               ...
                                                               bool isCategoryEnabled)
{
	...
	TrackIndex->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "TrackIndex"));
	TrackIndex->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	TrackIndex->Set(String::NewFromUtf8(isolate, "description"),
	...
	track1->Set(String::NewFromUtf8(isolate, "1"), String::NewFromUtf8(isolate,
	                                                                   "1"));
	TrackIndexValues->Set(0, track1);

	Local<Object> track2 = Object::New(isolate);
	track2->Set(String::NewFromUtf8(isolate, "2"), String::NewFromUtf8(isolate,
	                                                                   "2"));
	TrackIndexValues->Set(1, track2);

	Local<Object> track3 = Object::New(isolate);
	track3->Set(String::NewFromUtf8(isolate, "3"), String::NewFromUtf8(isolate,
	                                                                   "3"));
	TrackIndexValues->Set(2, track3);

	Local<Object> track4 = Object::New(isolate);
	track4->Set(String::NewFromUtf8(isolate, "4"), String::NewFromUtf8(isolate,
	                                                                   "4"));
	TrackIndexValues->Set(3, track4);

	Local<Object> track5 = Object::New(isolate);
	track5->Set(String::NewFromUtf8(isolate, "5"), String::NewFromUtf8(isolate,
	                                                                   "5"));
	TrackIndexValues->Set(4, track5);

	Local<Object> track6 = Object::New(isolate);
	track6->Set(String::NewFromUtf8(isolate, "6"), String::NewFromUtf8(isolate,
	                                                                   "6"));
	TrackIndexValues->Set(5, track6);

	TrackIndex->Set(String::NewFromUtf8(isolate, "values"), TrackIndexValues);

	const char *TrackIndexCurrentValue = config_get_string(config, "AdvOut",
	                                                       "TrackIndex");
	...
	TrackIndex->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	TrackIndex->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	TrackIndex->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));
	subCategoryParameters->Set(index++, TrackIndex);

	// Encoder : list
	Local<Object> EncoderList = Object::New(isolate);
	EncoderList->Set(String::NewFromUtf8(isolate, "name"),
	                 String::NewFromUtf8(isolate, "Encoder"));
	EncoderList->Set(String::NewFromUtf8(isolate, "type"),
	                 String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	EncoderList->Set(String::NewFromUtf8(isolate, "description"),
	                 String::NewFromUtf8(isolate, "Encoder"));

	const char *EncoderListCurrentValue = config_get_string(config, "AdvOut",
	                                                        "Encoder");
	...
	EncoderList->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                       true));
	EncoderList->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                       isCategoryEnabled));
	EncoderList->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                      false));
	...
	ApplyServiceSettings->Set(String::NewFromUtf8(isolate, "name"),
	                          String::NewFromUtf8(isolate, "ApplyServiceSettings"));
	ApplyServiceSettings->Set(String::NewFromUtf8(isolate, "type"),
	                          String::NewFromUtf8(isolate, "OBS_PROPERTY_BOOL"));
	...
	ApplyServiceSettings->Set(String::NewFromUtf8(isolate, "currentValue"),
	                          Integer::New(isolate, config_get_bool(config, "AdvOut",
	                                                                "ApplyServiceSettings")));

	ApplyServiceSettings->Set(String::NewFromUtf8(isolate, "visible"),
	                          Boolean::New(isolate, true));
	ApplyServiceSettings->Set(String::NewFromUtf8(isolate, "enabled"),
	                          Boolean::New(isolate, isCategoryEnabled));
	ApplyServiceSettings->Set(String::NewFromUtf8(isolate, "masked"),
	                          Boolean::New(isolate, false));
	subCategoryParameters->Set(index++, ApplyServiceSettings);

	// Rescale Output : boolean
	Local<Object> Rescale = Object::New(isolate);
	Rescale->Set(String::NewFromUtf8(isolate, "name"), String::NewFromUtf8(isolate,
	                                                                       "Rescale"));
	Rescale->Set(String::NewFromUtf8(isolate, "type"), String::NewFromUtf8(isolate,
	                                                                       "OBS_PROPERTY_BOOL"));
	Rescale->Set(String::NewFromUtf8(isolate, "description"),
	             String::NewFromUtf8(isolate, "Rescale Output"));
	bool doRescale = config_get_bool(config, "AdvOut", "Rescale");
	Rescale->Set(String::NewFromUtf8(isolate, "currentValue"), Integer::New(isolate,
	                                                                        doRescale));
	Rescale->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                   true));
	Rescale->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                   isCategoryEnabled));
	Rescale->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                  false));
	...
	if(doRescale) {
		...
		RescaleRes->Set(String::NewFromUtf8(isolate, "name"),
		                String::NewFromUtf8(isolate, "RescaleRes"));
		RescaleRes->Set(String::NewFromUtf8(isolate, "type"),
		                String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
		RescaleRes->Set(String::NewFromUtf8(isolate, "description"),
		                String::NewFromUtf8(isolate, "Output Resolution"));
		...
		RescaleRes->Set(String::NewFromUtf8(isolate, "currentValue"),
		                String::NewFromUtf8(isolate, outputResString));

		std::vector<pair<int, int>> outputResolutions = getOutputResolutions(base_cx,
		                                                                     base_cy);

		Local<Array> RescaleResValues = Array::New(isolate);
		for(int i = 0; i<outputResolutions.size(); i++) {
			string outRes = ResString(outputResolutions.at(i).first,
			                          outputResolutions.at(i).second);
			Local<Object> object = Object::New(isolate);
		...
		RescaleRes->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
		                                                                      true));
		RescaleRes->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
		                                                                      isCategoryEnabled));
		RescaleRes->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
		                                                                     false));
	...
	if(!fileExist) {
		streamingEncoder = obs_video_encoder_create(encoderID, "streaming_h264",
		                                            nullptr, nullptr);
	...
	getEncoderSettings(isolate, streamingEncoder, settings, &subCategoryParameters,
	                   index, isCategoryEnabled);

	subCategory->Set(String::NewFromUtf8(isolate, "nameSubCategory"),
	                 String::NewFromUtf8(isolate, "Streaming"));
	subCategory->Set(String::NewFromUtf8(isolate, "parameters"),
	                 subCategoryParameters);
...
void OBS_settings::getStandardRecordingSettings(Local<Array>
                                                *subCategoryParameters, config_t *config, bool isCategoryEnabled)
{
	Isolate *isolate = v8::Isolate::GetCurrent();
	int index = 1;
	...
	RecFilePath->Set(String::NewFromUtf8(isolate, "name"),
	                 String::NewFromUtf8(isolate, "RecFilePath"));
	RecFilePath->Set(String::NewFromUtf8(isolate, "type"),
	                 String::NewFromUtf8(isolate, "OBS_PROPERTY_PATH"));
	RecFilePath->Set(String::NewFromUtf8(isolate, "description"),
	                 String::NewFromUtf8(isolate, "Recording Path"));

	const char *RecFilePathCurrentValue = config_get_string(config, "AdvOut",
	                                                        "RecFilePath");
	...
	RecFilePath->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                       true));
	RecFilePath->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                       isCategoryEnabled));
	RecFilePath->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                      false));
	...
	RecFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "name"),
	                             String::NewFromUtf8(isolate, "RecFileNameWithoutSpace"));
	RecFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "type"),
	                             String::NewFromUtf8(isolate, "OBS_PROPERTY_BOOL"));
	...
	RecFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "visible"),
	                             Boolean::New(isolate, true));
	RecFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "enabled"),
	                             Boolean::New(isolate, isCategoryEnabled));
	RecFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "masked"),
	                             Boolean::New(isolate, false));
	(*subCategoryParameters)->Set(index++, RecFileNameWithoutSpace);

	// Recording Format : list
	Local<Object> RecFormat = Object::New(isolate);
	RecFormat->Set(String::NewFromUtf8(isolate, "name"),
	               String::NewFromUtf8(isolate, "RecFormat"));
	RecFormat->Set(String::NewFromUtf8(isolate, "type"),
	               String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	RecFormat->Set(String::NewFromUtf8(isolate, "description"),
	               String::NewFromUtf8(isolate, "Recording Format"));

	const char *RecFormatCurrentValue = config_get_string(config, "AdvOut",
	                                                      "RecFormat");
	...
	flv->Set(String::NewFromUtf8(isolate, "flv"), String::NewFromUtf8(isolate,
	                                                                  "flv"));
	RecFormatValues->Set(0, flv);

	Local<Object> mp4 = Object::New(isolate);
	mp4->Set(String::NewFromUtf8(isolate, "mp4"), String::NewFromUtf8(isolate,
	                                                                  "mp4"));
	RecFormatValues->Set(1, mp4);

	Local<Object> mov = Object::New(isolate);
	mov->Set(String::NewFromUtf8(isolate, "mov"), String::NewFromUtf8(isolate,
	                                                                  "mov"));
	RecFormatValues->Set(2, mov);

	Local<Object> mkv = Object::New(isolate);
	mkv->Set(String::NewFromUtf8(isolate, "mkv"), String::NewFromUtf8(isolate,
	                                                                  "mkv"));
	...
	m3u8->Set(String::NewFromUtf8(isolate, "m3u8"), String::NewFromUtf8(isolate,
	                                                                    "m3u8"));
	RecFormatValues->Set(5, m3u8);

	RecFormat->Set(String::NewFromUtf8(isolate, "values"), RecFormatValues);

	RecFormat->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                     true));
	RecFormat->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                     isCategoryEnabled));
	RecFormat->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                    false));
	(*subCategoryParameters)->Set(index++, RecFormat);

	// Audio Track : list
	Local<Object> RecTracks = Object::New(isolate);
	RecTracks->Set(String::NewFromUtf8(isolate, "name"),
	               String::NewFromUtf8(isolate, "RecTracks"));
	RecTracks->Set(String::NewFromUtf8(isolate, "type"),
	               String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	RecTracks->Set(String::NewFromUtf8(isolate, "description"),
	               String::NewFromUtf8(isolate, "Audio Track"));

	const char *RecTracksCurrentValue = config_get_string(config, "AdvOut",
	                                                      "RecTracks");
	...
	RecTracks->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                     true));
	RecTracks->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                     isCategoryEnabled));
	RecTracks->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                    false));
	(*subCategoryParameters)->Set(index++, RecTracks);

	// Encoder : list
	Local<Object> RecEncoder = Object::New(isolate);
	RecEncoder->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "RecEncoder"));
	RecEncoder->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	RecEncoder->Set(String::NewFromUtf8(isolate, "description"),
	                String::NewFromUtf8(isolate, "Recording"));

	const char *RecEncoderCurrentValue = config_get_string(config, "AdvOut",
	                                                       "RecEncoder");
	if(RecEncoderCurrentValue == NULL
	            || strcmp(RecEncoderCurrentValue, "none") == 0)
	...
	RecEncoder->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	RecEncoder->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	RecEncoder->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));
	(*subCategoryParameters)->Set(index++, RecEncoder);

	// Rescale Output : boolean
	Local<Object> RecRescale = Object::New(isolate);
	RecRescale->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "RecRescale"));
	RecRescale->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_BOOL"));
	RecRescale->Set(String::NewFromUtf8(isolate, "description"),
	                String::NewFromUtf8(isolate, "Rescale Output"));
	...
	RecRescale->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	RecRescale->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	RecRescale->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));
	...
	if(doRescale) {
		...
		RecRescaleRes->Set(String::NewFromUtf8(isolate, "name"),
		                   String::NewFromUtf8(isolate, "RecRescaleRes"));
		RecRescaleRes->Set(String::NewFromUtf8(isolate, "type"),
		                   String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
		RecRescaleRes->Set(String::NewFromUtf8(isolate, "description"),
		                   String::NewFromUtf8(isolate, "Output Resolution"));

		uint32_t base_cx = config_get_uint(config, "Video", "BaseCX");
		uint32_t base_cy = config_get_uint(config, "Video", "BaseCY");

		const char *outputResString = config_get_string(config, "AdvOut",
		                                                "RecRescaleRes");
		...
		RecRescaleRes->Set(String::NewFromUtf8(isolate, "currentValue"),
		                   String::NewFromUtf8(isolate, outputResString));
		...
		RecRescaleRes->Set(String::NewFromUtf8(isolate, "visible"),
		                   Boolean::New(isolate, true));
		RecRescaleRes->Set(String::NewFromUtf8(isolate, "enabled"),
		                   Boolean::New(isolate, isCategoryEnabled));
		RecRescaleRes->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
		                                                                        false));
	...
	RecMuxerCustom->Set(String::NewFromUtf8(isolate, "name"),
	                    String::NewFromUtf8(isolate, "RecMuxerCustom"));
	RecMuxerCustom->Set(String::NewFromUtf8(isolate, "type"),
	                    String::NewFromUtf8(isolate, "OBS_PROPERTY_EDIT_TEXT"));
	RecMuxerCustom->Set(String::NewFromUtf8(isolate, "description"),
	                    String::NewFromUtf8(isolate, "Custom Muxer Settings"));

	const char *RecMuxerCustomCurrentValue = config_get_string(config, "AdvOut",
	                                                           "RecMuxerCustom");
	...
	RecMuxerCustom->Set(String::NewFromUtf8(isolate, "visible"),
	                    Boolean::New(isolate, true));
	RecMuxerCustom->Set(String::NewFromUtf8(isolate, "enabled"),
	                    Boolean::New(isolate, isCategoryEnabled));
	RecMuxerCustom->Set(String::NewFromUtf8(isolate, "masked"),
	                    Boolean::New(isolate, false));
	...
	if(!fileExist) {
		recordingEncoder =
		      obs_video_encoder_create(RecEncoderCurrentValue, "streaming_h264", nullptr,
		                               nullptr);
		...
		obs_data_apply(settings, data);
		recordingEncoder =
		      obs_video_encoder_create(RecEncoderCurrentValue, "streaming_h264", settings,
		                               nullptr);
		OBS_service::setRecordingEncoder(recordingEncoder);
	}

	getEncoderSettings(isolate, recordingEncoder, settings, subCategoryParameters,
	                   index, isCategoryEnabled);
}
...
void OBS_settings::getFFmpegOutputRecordingSettings(Local<Array>
                                                    *subCategoryParameters, config_t *config, bool isCategoryEnabled)
{
	Isolate *isolate = v8::Isolate::GetCurrent();
	int index = 1;
	...
	FFOutputToFile->Set(String::NewFromUtf8(isolate, "name"),
	                    String::NewFromUtf8(isolate, "FFOutputToFile"));
	FFOutputToFile->Set(String::NewFromUtf8(isolate, "type"),
	                    String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	FFOutputToFile->Set(String::NewFromUtf8(isolate, "description"),
	                    String::NewFromUtf8(isolate, "FFmpeg Output Type"));
	bool FFOutputToFileCurrentValue = config_get_bool(config, "AdvOut",
	                                                  "FFOutputToFile");
	...
	file->Set(String::NewFromUtf8(isolate, "Output to File"), Integer::New(isolate,
	                                                                       true));
	FFOutputToFileValues->Set(0, file);

	Local<Object> url = Object::New(isolate);
	url->Set(String::NewFromUtf8(isolate, "Output to URL"), Integer::New(isolate,
	                                                                     false));
	FFOutputToFileValues->Set(1, url);

	FFOutputToFile->Set(String::NewFromUtf8(isolate, "values"),
	                    FFOutputToFileValues);

	FFOutputToFile->Set(String::NewFromUtf8(isolate, "visible"),
	                    Boolean::New(isolate, true));
	FFOutputToFile->Set(String::NewFromUtf8(isolate, "enabled"),
	                    Boolean::New(isolate, isCategoryEnabled));
	FFOutputToFile->Set(String::NewFromUtf8(isolate, "masked"),
	                    Boolean::New(isolate, false));
	...
	if(FFOutputToFileCurrentValue) {
		...
		FFfile->Set(String::NewFromUtf8(isolate, "name"), String::NewFromUtf8(isolate,
		                                                                      "FFFilePath"));
		FFfile->Set(String::NewFromUtf8(isolate, "type"), String::NewFromUtf8(isolate,
		                                                                      "OBS_PROPERTY_PATH"));
		FFfile->Set(String::NewFromUtf8(isolate, "description"),
		            String::NewFromUtf8(isolate, "File path"));
		const char *FFfileCurrentValue = config_get_string(config, "AdvOut",
		                                                   "FFFilePath");
		...
		FFfile->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
		                                                                  true));
		FFfile->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
		                                                                  isCategoryEnabled));
		FFfile->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
		                                                                 false));
		(*subCategoryParameters)->Set(index++, FFfile);
	} else {
		Local<Object> FFURL = Object::New(isolate);
		FFURL->Set(String::NewFromUtf8(isolate, "name"), String::NewFromUtf8(isolate,
		                                                                     "FFURL"));
		FFURL->Set(String::NewFromUtf8(isolate, "type"), String::NewFromUtf8(isolate,
		...
		FFURL->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
		                                                                 true));
		FFURL->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
		                                                                 isCategoryEnabled));
		FFURL->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
		                                                                false));
	...
	FFFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "name"),
	                            String::NewFromUtf8(isolate, "FFFileNameWithoutSpace"));
	FFFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "type"),
	                            String::NewFromUtf8(isolate, "OBS_PROPERTY_BOOL"));
	...
	FFFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "visible"),
	                            Boolean::New(isolate, true));
	FFFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "enabled"),
	                            Boolean::New(isolate, isCategoryEnabled));
	FFFileNameWithoutSpace->Set(String::NewFromUtf8(isolate, "masked"),
	                            Boolean::New(isolate, false));
	...
	FFFormat->Set(String::NewFromUtf8(isolate, "name"), String::NewFromUtf8(isolate,
	                                                                        "FFFormat"));
	FFFormat->Set(String::NewFromUtf8(isolate, "type"), String::NewFromUtf8(isolate,
	                                                                        "OBS_PROPERTY_LIST"));
	FFFormat->Set(String::NewFromUtf8(isolate, "description"),
	              String::NewFromUtf8(isolate, "Container Format"));

	const char *FFFormatCurrentValue = config_get_string(config, "AdvOut",
	                                                     "FFFormat");
	...
	FFFormat->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                    true));
	FFFormat->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                    isCategoryEnabled));
	FFFormat->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                   false));
	...
	FFMCustom->Set(String::NewFromUtf8(isolate, "name"),
	               String::NewFromUtf8(isolate, "FFMCustom"));
	FFMCustom->Set(String::NewFromUtf8(isolate, "type"),
	               String::NewFromUtf8(isolate, "OBS_PROPERTY_EDIT_TEXT"));
	FFMCustom->Set(String::NewFromUtf8(isolate, "description"),
	               String::NewFromUtf8(isolate, "Muxer Settings (if any)"));
	const char *FFMCustomCurrentValue = config_get_string(config, "AdvOut",
	                                                      "FFMCustom");
	...
	FFMCustom->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                     true));
	FFMCustom->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                     isCategoryEnabled));
	FFMCustom->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                    false));
	(*subCategoryParameters)->Set(index++, FFMCustom);

	// Video Bitrate : int 													FFVBitrate
	Local<Object> FFVBitrate = Object::New(isolate);
	FFVBitrate->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "FFVBitrate"));
	FFVBitrate->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_INT"));
	...
	FFVBitrate->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	FFVBitrate->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	FFVBitrate->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));
	...
	FFVGOPSize->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "FFVGOPSize"));
	FFVGOPSize->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_INT"));
	...
	FFVGOPSize->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	FFVGOPSize->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	FFVGOPSize->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));
	(*subCategoryParameters)->Set(index++, FFVGOPSize);

	// Rescale Output : boolean												FFRescale
	Local<Object> FFRescale = Object::New(isolate);
	FFRescale->Set(String::NewFromUtf8(isolate, "name"),
	               String::NewFromUtf8(isolate, "FFRescale"));
	FFRescale->Set(String::NewFromUtf8(isolate, "type"),
	               String::NewFromUtf8(isolate, "OBS_PROPERTY_BOOL"));
	...
	FFRescale->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                     true));
	FFRescale->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                     isCategoryEnabled));
	FFRescale->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                    false));
	...
	if(FFRescaleCurrentValue) {
		...
		FFRescaleRes->Set(String::NewFromUtf8(isolate, "name"),
		                  String::NewFromUtf8(isolate, "FFRescaleRes"));
		FFRescaleRes->Set(String::NewFromUtf8(isolate, "type"),
		                  String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
		FFRescaleRes->Set(String::NewFromUtf8(isolate, "description"),
		                  String::NewFromUtf8(isolate, "Output resolutions"));
		FFRescaleRes->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
		                                                                        true));
		FFRescaleRes->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
		                                                                        isCategoryEnabled));
		FFRescaleRes->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
		                                                                       false));
	...
	FFIgnoreCompat->Set(String::NewFromUtf8(isolate, "name"),
	                    String::NewFromUtf8(isolate, "FFIgnoreCompat"));
	FFIgnoreCompat->Set(String::NewFromUtf8(isolate, "type"),
	                    String::NewFromUtf8(isolate, "OBS_PROPERTY_BOOL"));
	FFIgnoreCompat->Set(String::NewFromUtf8(isolate, "description"),
	                    String::NewFromUtf8(isolate,
	                                        "Show all codes (even if potentially incompatible)"));
	...
	FFIgnoreCompat->Set(String::NewFromUtf8(isolate, "visible"),
	                    Boolean::New(isolate, true));
	FFIgnoreCompat->Set(String::NewFromUtf8(isolate, "enabled"),
	                    Boolean::New(isolate, isCategoryEnabled));
	FFIgnoreCompat->Set(String::NewFromUtf8(isolate, "masked"),
	                    Boolean::New(isolate, false));
	...
	FFVEncoder->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "FFVEncoder"));
	FFVEncoder->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	FFVEncoder->Set(String::NewFromUtf8(isolate, "description"),
	                String::NewFromUtf8(isolate, "Video Encoder"));

	const char *FFVEncoderCurrentValue = config_get_string(config, "AdvOut",
	                                                       "FFVEncoder");
	...
	FFAEncoder->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "FFAEncoder"));
	FFAEncoder->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	FFAEncoder->Set(String::NewFromUtf8(isolate, "description"),
	                String::NewFromUtf8(isolate, "Audio Encoder"));

	const char *FFAEncoderCurrentValue = config_get_string(config, "AdvOut",
	                                                       "FFAEncoder");
	...
	FFVEncoder->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	FFVEncoder->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	FFVEncoder->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));

	FFAEncoder->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	// FFAEncoder->Set(String::NewFromUtf8(isolate, "enabled"), Integer::New(isolate, codec_has_audio));
	FFAEncoder->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	FFAEncoder->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));

	// Video Encoder Settings (if any) : edit text 							FFVCustom
	Local<Object> FFVCustom = Object::New(isolate);
	FFVCustom->Set(String::NewFromUtf8(isolate, "name"),
	               String::NewFromUtf8(isolate, "FFVCustom"));
	FFVCustom->Set(String::NewFromUtf8(isolate, "type"),
	               String::NewFromUtf8(isolate, "OBS_PROPERTY_EDIT_TEXT"));
	FFVCustom->Set(String::NewFromUtf8(isolate, "description"),
	               String::NewFromUtf8(isolate, "Video Encoder Settings (if any)"));
	const char *FFVCustomCurrentValue = config_get_string(config, "AdvOut",
	                                                      "FFVCustom");
	...
	FFVCustom->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                     true));
	FFVCustom->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                     isCategoryEnabled));
	FFVCustom->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                    false));

	// Audio Bitrate : list 												FFABitrate
	Local<Object> FFABitrate = Object::New(isolate);
	FFABitrate->Set(String::NewFromUtf8(isolate, "name"),
	                String::NewFromUtf8(isolate, "FFABitrate"));
	FFABitrate->Set(String::NewFromUtf8(isolate, "type"),
	                String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	...
	FFABitrate->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                      true));
	// FFABitrate->Set(String::NewFromUtf8(isolate, "enabled"), Integer::New(isolate, codec_has_audio));
	FFABitrate->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                      isCategoryEnabled));
	FFABitrate->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                     false));

	// Audio track : list 													FFAudioTrack
	Local<Object> FFAudioTrack = Object::New(isolate);
	FFAudioTrack->Set(String::NewFromUtf8(isolate, "name"),
	                  String::NewFromUtf8(isolate, "FFAudioTrack"));
	FFAudioTrack->Set(String::NewFromUtf8(isolate, "type"),
	                  String::NewFromUtf8(isolate, "OBS_PROPERTY_LIST"));
	...
	FFAudioTrack->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                        true));
	// FFAudioTrack->Set(String::NewFromUtf8(isolate, "enabled"), Integer::New(isolate, codec_has_audio));
	FFAudioTrack->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                        isCategoryEnabled));
	FFAudioTrack->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                       false));

	// Audio Encoder Settings (if any) : edit text 							FFACustom
	Local<Object> FFACustom = Object::New(isolate);
	FFACustom->Set(String::NewFromUtf8(isolate, "name"),
	               String::NewFromUtf8(isolate, "FFACustom"));
	FFACustom->Set(String::NewFromUtf8(isolate, "type"),
	               String::NewFromUtf8(isolate, "OBS_PROPERTY_EDIT_TEXT"));
	FFACustom->Set(String::NewFromUtf8(isolate, "description"),
	               String::NewFromUtf8(isolate, "Audio Encoder Settings (if any)"));

	const char *FFACustomCurrentValue = config_get_string(config, "AdvOut",
	                                                      "FFACustom");
	...
	FFACustom->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                     true));
	// FFACustom->Set(String::NewFromUtf8(isolate, "enabled"), Integer::New(isolate, codec_has_audio));
	FFACustom->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                     isCategoryEnabled));
	FFACustom->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                    false));
                                                               ...
                                                               bool isCategoryEnabled)
{
	...
	RecType->Set(String::NewFromUtf8(isolate, "name"), String::NewFromUtf8(isolate,
	                                                                       "RecType"));
	RecType->Set(String::NewFromUtf8(isolate, "type"), String::NewFromUtf8(isolate,
	                                                                       "OBS_PROPERTY_LIST"));
	RecType->Set(String::NewFromUtf8(isolate, "description"),
	             String::NewFromUtf8(isolate, "Type"));
	Local<Array> RecTypeValues = Array::New(isolate);

	Local<Object> Standard = Object::New(isolate);
	Standard->Set(String::NewFromUtf8(isolate, "Standard"),
	              String::NewFromUtf8(isolate, "Standard"));
	RecTypeValues->Set(0, Standard);

	Local<Object> Custom = Object::New(isolate);
	Custom->Set(String::NewFromUtf8(isolate, "Custom Output (FFmpeg)"),
	            String::NewFromUtf8(isolate, "Custom Output (FFmpeg)"));
	RecTypeValues->Set(1, Custom);

	RecType->Set(String::NewFromUtf8(isolate, "values"), RecTypeValues);

	const char *RecTypeCurrentValue = config_get_string(config, "AdvOut",
	                                                    "RecType");
	...
	RecType->Set(String::NewFromUtf8(isolate, "visible"), Boolean::New(isolate,
	                                                                   true));
	RecType->Set(String::NewFromUtf8(isolate, "enabled"), Boolean::New(isolate,
	                                                                   isCategoryEnabled));
	RecType->Set(String::NewFromUtf8(isolate, "masked"), Boolean::New(isolate,
	                                                                  false));
...
void OBS_settings::getAdvancedOutputAudioSettings(Local<Array> outputSettings,
                                                  config_t *config, bool isCategoryEnabled)
{
	std::vector<std::vector<std::pair<std::string, std::string>>> entries;
	...
	for (auto &entry : bitrateMap)
		Track1Bitrate.push_back(std::make_pair(std::to_string(entry.first),
		                                       std::to_string(entry.first)));
	...
	outputSettings->Set(3, serializeSettingsData("Audio - Track 1", entries, config,
	                                             "AdvOut", true, isCategoryEnabled));
	entries.clear();
	...
	for (auto &entry : bitrateMap)
		Track2Bitrate.push_back(std::make_pair(std::to_string(entry.first),
		                                       std::to_string(entry.first)));
	...
	outputSettings->Set(4, serializeSettingsData("Audio - Track 2", entries, config,
	                                             "AdvOut", true, isCategoryEnabled));
	entries.clear();
	...
	for (auto &entry : bitrateMap)
		Track3Bitrate.push_back(std::make_pair(std::to_string(entry.first),
		                                       std::to_string(entry.first)));
	...
	outputSettings->Set(5, serializeSettingsData("Audio - Track 3", entries, config,
	                                             "AdvOut", true, isCategoryEnabled));
	entries.clear();
	...
	for (auto &entry : bitrateMap)
		Track4Bitrate.push_back(std::make_pair(std::to_string(entry.first),
		                                       std::to_string(entry.first)));
	...
	outputSettings->Set(6, serializeSettingsData("Audio - Track 4", entries, config,
	                                             "AdvOut", true, isCategoryEnabled));
	entries.clear();
	...
	for (auto &entry : bitrateMap)
		Track5Bitrate.push_back(std::make_pair(std::to_string(entry.first),
		                                       std::to_string(entry.first)));
	...
	outputSettings->Set(7, serializeSettingsData("Audio - Track 5", entries, config,
	                                             "AdvOut", true, isCategoryEnabled));
	entries.clear();
	...
	for (auto &entry : bitrateMap)
		Track6Bitrate.push_back(std::make_pair(std::to_string(entry.first),
		                                       std::to_string(entry.first)));
	...
	outputSettings->Set(8, serializeSettingsData("Audio - Track 6", entries, config,
	                                             "AdvOut", true, isCategoryEnabled));
	entries.clear();
}

void OBS_settings::getAdvancedOutputSettings(Local<Array> outputSettings,
                                             config_t *config, bool isCategoryEnabled)
{
	std::vector<std::vector<std::pair<std::string, std::string>>> entries;
...
Local<Array> OBS_settings::getOutputSettings()
{
	...
	outputSettings->Set(0, serializeSettingsData("Untitled", entries, config,
	                                             "Output", true, isCategoryEnabled));
	entries.clear();
...
void OBS_settings::saveSimpleOutputSettings(Local<Array> settings,
                                            std::string basicConfigFile)
{
	saveGenericSettings(settings, "SimpleOutput", basicConfigFile);
}

void OBS_settings::saveAdvancedOutputStreamingSettings(Local<Array> settings,
                                                       std::string basicConfigFile)
{
	config_t *config = OBS_API::openConfigFile(basicConfigFile);
	Isolate *isolate = v8::Isolate::GetCurrent();
	int indexStreamingCategory = 1;
	Local<Array> streamSettings = v8::Local<v8::Array>::Cast(settings->Get(
	                                                               indexStreamingCategory));
	std::string section = "AdvOut";

	Local<Array> parameters = v8::Local<v8::Array>::Cast(streamSettings->Get(
	                                                           String::NewFromUtf8(isolate, "parameters")));
		...
		if(type.compare("OBS_PROPERTY_LIST") == 0 ||
			...
			if(i < indexEncoderSettings) {
				...
				if (name.compare("Encoder") == 0) {
					const char *currentEncoder = config_get_string(config, section.c_str(),
					                                               name.c_str());
	...
	if (newEncoderType) encoderSettings = obs_encoder_defaults(config_get_string(
		                                                                 config, section.c_str(), "Encoder"));
...
void OBS_settings::saveAdvancedOutputRecordingSettings(Local<Array> settings,
                                                       std::string basicConfigFile)
{
	config_t *config = OBS_API::openConfigFile(basicConfigFile);
	Isolate *isolate = v8::Isolate::GetCurrent();
	int indexStreamingCategory = 2;
	Local<Array> recordSettings = v8::Local<v8::Array>::Cast(settings->Get(
	                                                               indexStreamingCategory));
	std::string section = "AdvOut";

	Local<Array> parameters =
	      v8::Local<v8::Array>::Cast(recordSettings->Get(String::NewFromUtf8(isolate,
	                                                                         "parameters")));
	...
	if (newEncoderType) encoderSettings = obs_encoder_defaults(config_get_string(
		                                                                 config, section.c_str(), "RecEncoder"));
...
void OBS_settings::saveAdvancedOutputSettings(Local<Array> settings,
                                              std::string basicConfigFile)
{
	// Streaming
...
void OBS_settings::saveOutputSettings(Local<Array> settings)
{
	...
	Local<Array> parameters = v8::Local<v8::Array>::Cast(outputModeObject->Get(
	                                                           String::NewFromUtf8(isolate, "parameters")));
...
static void LoadListValues(obs_property_t *prop, int index,
                           std::vector<std::pair<std::string, std::string>> *listValues)
{
...
static void LoadAudioDevices(std::vector<std::pair<std::string, std::string>>
                             *desktopAudioDevice,
...
Local<Array> OBS_settings::getAudioSettings()
{
	...
	desktopAudioDevice.push_back(std::make_pair("description",
	                                            "Desktop Audio Device"));
	...
	desktopAudioDevice2.push_back(std::make_pair("description",
	                                             "Desktop Audio Device 2"));
	...
	auxAudioDevice.push_back(std::make_pair("description",
	                                        "Mic/Auxiliary Audio Device"));
	...
	auxAudioDevice2.push_back(std::make_pair("description",
	                                         "Mic/Auxiliary Audio Device 2"));
	...
	auxAudioDevice3.push_back(std::make_pair("description",
	                                         "Mic/Auxiliary Audio Device 3"));
	...
	audioSettings->Set(0, serializeSettingsData("Untitled", entries, config,
	                                            "Audio", true, true));
	entries.clear();
...
void OBS_settings::saveAudioSettings(Local<Array> audioSettings)
{
	...
	Local<Object> audioSettingsObject = v8::Local<v8::Object>::Cast(
	                                          audioSettings->Get(0));
	Local<Array> parameters = v8::Local<v8::Array>::Cast(audioSettingsObject->Get(
	                                                           String::NewFromUtf8(isolate, "parameters")));
	...
	for(int i=2; i<parameters->Length(); i++) {
		...
		v8::String::Utf8Value param0(audioDevice->Get(String::NewFromUtf8(isolate,
		                                                                  "name")));
		v8::String::Utf8Value param1(audioDevice->Get(String::NewFromUtf8(isolate,
		                                                                  "currentValue")));
...
std::vector<pair<int, int>> OBS_settings::getOutputResolutions(int base_cx,
                                                               int base_cy)
{
...
Local<Array>  OBS_settings::getVideoSettings()
{
	...
	baseResolution.push_back(std::make_pair("description",
	                                        "Base (Canvas) Resolution"));
	...
	for(int i=0; i<resolutions.size(); i++) {
		...
		pair<std::string, std::string> newBaseResolution = std::make_pair(
		                                                         baseResolutionString.c_str(), baseResolutionString.c_str());
	...
	pair<std::string, std::string> newBaseResolution =
	      std::make_pair("currentValue", baseResolutionString);
	...
	if (it == baseResolution.end())
		baseResolution.push_back(std::make_pair(baseResolutionString.c_str(),
		                                        baseResolutionString.c_str()));

	int indexFirstValue = 3;
	baseResolution.insert(baseResolution.begin() + indexFirstValue,
	                      newBaseResolution);
	...
	outputResolution.push_back(std::make_pair("description",
	...
	scaleType.push_back(std::make_pair("Bilinear (Fastest, but blurry if scaling)",
	                                   "bilinear"));
	scaleType.push_back(std::make_pair("Bicubic (Sharpened scaling, 16 samples)",
	                                   "bicubic"));
	scaleType.push_back(std::make_pair("Lanczos (Sharpened scaling, 32 samples)",
	                                   "lanczos"));
	...
	if(fpsTypeValue == 0) {
		...
		fpsType.push_back(std::make_pair("Fractional FPS Value",
	...
	videoSettings->Set(0, serializeSettingsData("Untitled", entries, config,
	                                            "Video", true, isCategoryEnabled));
	entries.clear();
...
void OBS_settings::saveVideoSettings(Local<Array> videoSettings)
{
	...
	Local<Object> videoSettingsObject = v8::Local<v8::Object>::Cast(
	                                          videoSettings->Get(0));
	Local<Array> parameters = v8::Local<v8::Array>::Cast(videoSettingsObject->Get(
	                                                           String::NewFromUtf8(isolate, "parameters")));

	//Base resolution
	Local<Object> baseRes = v8::Local<v8::Object>::Cast(parameters->Get(0));

	v8::String::Utf8Value param0(baseRes->Get(String::NewFromUtf8(isolate,
	                                                              "currentValue")));
	...
	v8::String::Utf8Value param1(outputRes->Get(String::NewFromUtf8(isolate,
	                                                                "currentValue")));
	...
	v8::String::Utf8Value param2(scaleParameter->Get(String::NewFromUtf8(isolate,
	                                                                     "currentValue")));
	...
	v8::String::Utf8Value param3(fpsType->Get(String::NewFromUtf8(isolate,
	                                                              "currentValue")));
	...
	if(fpsTypeString.compare("Common FPS Values") == 0) {
		...
		v8::String::Utf8Value param4(fpsCommon->Get(String::NewFromUtf8(isolate,
		                                                                "currentValue")));
		...
		v8::Local<v8::Integer> fpsIntValue = v8::Local<v8::Integer>::Cast(fpsInt->Get(
		                                                                        String::NewFromUtf8(isolate, "currentValue")));
		...
		v8::Local<v8::Integer> fpsNumValue = v8::Local<v8::Integer>::Cast(fpsNum->Get(
		                                                                        String::NewFromUtf8(isolate, "currentValue")));
		...
		if(parameters->Length() > 5) {
			...
			v8::Local<v8::Integer> fpsDenValue = v8::Local<v8::Integer>::Cast(fpsDen->Get(
			                                                                        String::NewFromUtf8(isolate, "currentValue")));
...
Local<Array>  OBS_settings::getAdvancedSettings()
{
	...
	const char *processPriorityCurrentValue = config_get_string(globalConfig,
	...
	advancedSettings->Set(0, serializeSettingsData("General", entries, globalConfig,
	                                               "General", true, true));
	entries.clear();
...
#endif
	//Video
	const char *videoColorFormat = config_get_string(config, "Video",
	                                                 "ColorFormat");
	...
	advancedSettings->Set(1, serializeSettingsData("Video", entries, config,
	                                               "Video", true, true));
	entries.clear();

#if defined(_WIN32) || defined(__APPLE__)
	//Audio
	const char *monDevName = config_get_string(config, "Audio",
	                                           "MonitoringDeviceName");
	const char *monDevId = config_get_string(config, "Audio", "MonitoringDeviceId");

	//Audio Monitoring Device
	std::vector<std::pair<std::string, std::string>> *monitoringDevice = new
	...
	monitoringDevice->push_back(std::make_pair("description",
	                                           "Audio Monitoring Device"));
	...
	disableAudioDucking.push_back(std::make_pair("description",
	                                             "Disable Windows audio ducking"));
	entries.push_back(disableAudioDucking);

	advancedSettings->Set(2, serializeSettingsData("Audio", entries, config,
	                                               "Audio", true, true));
	entries.clear();
#endif
	...
	filenameFormatting.push_back(std::make_pair("description",
	                                            "Filename Formatting"));
	...
	overwriteIfExists.push_back(std::make_pair("description",
	                                           "Overwrite if file exists"));
	...
	recRBPrefix.push_back(std::make_pair("description",
	                                     "Replay Buffer Filename Prefix"));
	...
	recRBSuffix.push_back(std::make_pair("description",
	                                     "Replay Buffer Filename Suffix"));
	entries.push_back(recRBSuffix);

	advancedSettings->Set(3, serializeSettingsData("Recording", entries, config,
	                                               "SimpleOutput", true, true));
	entries.clear();
	...
	delayPreserve.push_back(std::make_pair("description",
	                                       "Preserved cutoff point (increase delay) when reconnecting"));
	entries.push_back(delayPreserve);

	advancedSettings->Set(4, serializeSettingsData("Stream Delay", entries, config,
	                                               "Output", true, true));
	entries.clear();
	...
	advancedSettings->Set(5, serializeSettingsData("Automatically Reconnect",
	                                               entries, config, "Output", true, true));
	entries.clear();
	...
	newSocketLoopEnable.push_back(std::make_pair("description",
	                                             "Enable new networking code"));
	...
	advancedSettings->Set(6, serializeSettingsData("Network", entries, config,
	                                               "Output", true, true));
	entries.clear();
...
void OBS_settings::saveGenericSettings(Local<Array> genericSettings,
                                       std::string section, std::string pathFile)
{
	Isolate *isolate = v8::Isolate::GetCurrent();
		...
		for(int j=0; j<parameters->Length(); j++) {
			...
			if(type.compare("OBS_PROPERTY_LIST") == 0 ||
				...
				if(name.compare("MonitoringDeviceName") == 0) {
					...
					config_set_string(config, section.c_str(), "MonitoringDeviceName",
					                  monDevName.c_str());
					config_set_string(config, section.c_str(), "MonitoringDeviceId",
					                  monDevId.c_str());
				} else
					config_set_string(config, section.c_str(), name.c_str(), value.c_str());
			} else if(type.compare("OBS_PROPERTY_INT") == 0) {
