...
void OBS_service::OBS_service_resetAudioContext(const
                                                FunctionCallbackInfo<Value> &args)
{
	resetAudioContext();
}

void OBS_service::OBS_service_resetVideoContext(const
                                                FunctionCallbackInfo<Value> &args)
{
	resetVideoContext(NULL);
}

void OBS_service::OBS_service_createAudioEncoder(const
                                                 FunctionCallbackInfo<Value> &args)
{
	createAudioEncoder();
}
...
void OBS_service::OBS_service_createService(const FunctionCallbackInfo<Value>
                                            &args)
{
	createService();
}

void OBS_service::OBS_service_createRecordingSettings(const
                                                      FunctionCallbackInfo<Value> &args)
{
	createRecordingSettings();
}

void OBS_service::OBS_service_createStreamingOutput(const
                                                    FunctionCallbackInfo<Value> &args)
{
	createStreamingOutput();
}

void OBS_service::OBS_service_createRecordingOutput(const
                                                    FunctionCallbackInfo<Value> &args)
{
	createRecordingOutput();
}

void OBS_service::OBS_service_startStreaming(const FunctionCallbackInfo<Value>
                                             &args)
{
	startStreaming();
}

void OBS_service::OBS_service_startRecording(const FunctionCallbackInfo<Value>
                                             &args)
{
	startRecording();
}

void OBS_service::OBS_service_stopStreaming(const FunctionCallbackInfo<Value>
                                            &args)
{
	stopStreaming();
}

void OBS_service::OBS_service_stopRecording(const FunctionCallbackInfo<Value>
                                            &args)
{
	stopRecording();
}
...
void OBS_service::OBS_service_setRecordingSettings(const
                                                   FunctionCallbackInfo<Value> &args)
{
	setRecordingSettings();
}

void OBS_service::OBS_service_isStreamingOutputActive(const
                                                      FunctionCallbackInfo<Value> &args)
{
...
bool OBS_service::resetVideoContext(const char *outputType)
{
	...
	ovi.output_width   = (uint32_t)config_get_uint(basicConfig, "Video",
	                                               "OutputCX");
	ovi.output_height  = (uint32_t)config_get_uint(basicConfig, "Video",
	                                               "OutputCY");
	...
	if (ovi.base_width == 0 || ovi.base_height == 0) {
		for (int i = 0; i<resolutions.size(); i++) {
			if (ovi.base_width * ovi.base_height < resolutions.at(i).width * resolutions.at(
			                  i).height) {
	...
	ovi.colorspace     = astrcmpi(colorSpace,
	                              "601") == 0 ? VIDEO_CS_601 : VIDEO_CS_709;
	ovi.range          = astrcmpi(colorRange,
	                              "Full") == 0 ? VIDEO_RANGE_FULL : VIDEO_RANGE_PARTIAL;
...
void OBS_service::createAudioEncoder(void)
{
	...
	audioEncoder = obs_audio_encoder_create(id, "simple_audio", nullptr, 0,
	                                        nullptr);
}
...
void OBS_service::createVideoStreamingEncoder()
{
	...
	config_t *config;
	int result = config_open(&config, basicConfigFile.c_str(),
	...
	videoStreamingEncoder = obs_video_encoder_create(encoder, "streaming_h264",
	                                                 nullptr, nullptr);
...
void OBS_service::createVideoRecordingEncoder()
{
	...
	videoRecordingEncoder = obs_video_encoder_create("obs_x264",
...
{
	streamingOutput = obs_output_create("rtmp_output", "simple_stream", nullptr,
	                                    nullptr);
}

void OBS_service::createRecordingOutput(void)
{
	recordingOutput = obs_output_create("ffmpeg_muxer", "simple_file_output",
	                                    nullptr, nullptr);
...
void OBS_service::associateAudioAndVideoEncodersToTheCurrentStreamingOutput(
      void)
{
...
void OBS_service::associateAudioAndVideoEncodersToTheCurrentRecordingOutput(
      void)
{
...
void OBS_service::updateVideoStreamingEncoder()
{
	...
	if(encoder != NULL) {
		...
		if(strcmp(obs_encoder_get_id(videoStreamingEncoder), encoderID) != 0) {
			if(videoStreamingEncoder != NULL)
				obs_encoder_release(videoStreamingEncoder);
			videoStreamingEncoder = obs_video_encoder_create(encoderID, "streaming_h264",
			                                                 nullptr, nullptr);
		}
	}
...
void OBS_service::updateRecordingOutput(void)
{
	...
	if(filenameFormat != NULL && format != NULL) {
		strPath += GenerateSpecifiedFilename(ffmpegOutput ? "avi" : format, noSpace,
		                                     filenameFormat);
		if(!strPath.empty())
...
void OBS_service::updateStreamSettings(void)
{
	...
	if(strcmp(currentOutputMode, "Simple") == 0)
		...
		if (applyServiceSettings) {
			...
			obs_service_apply_encoder_settings(OBS_service::getService(), encoderSettings,
			                                   nullptr);
		}
	}
