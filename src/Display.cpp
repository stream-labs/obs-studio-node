...
static LRESULT CALLBACK DisplayWndProc(_In_ HWND hwnd, _In_ UINT uMsg,
...
static void DrawGlyph(VertexBufferHelper *vb, float_t x, float_t y,
                      float_t scale, float_t depth, char glyph, uint32_t color)
{
...
inline static
void DrawBox(float_t x, float_t y, float_t w, float_t h, float_t depth,
             uint32_t color, VertexBufferHelper *vbh)
{
	VertexHelper *v;
...
inline static
void DrawBoxOutline(float_t x, float_t y, float_t w, float_t h, float_t depth,
                    uint32_t color, VertexBufferHelper *vbh)
{
	VertexHelper *v;
...
bool Display::DrawSelectedSource(obs_scene_t *scene, obs_sceneitem_t *item,
                                 void *param)
{
	...
	if (obs_sceneitem_selected(item) && !isOnlyAudio && ((itemWidth > 0)
		...
		for (size_t n = 0; n < 4; n++) {
			bool isIn = (edge[n].x >= 0)
			            && (edge[n].x < sceneWidth * dp->m_worldToPreviewScale.x)
			...
			if (left > 0.5) { // LEFT
				...
				if (dist > (pt * 4)) {
					...
					size_t len = (size_t)snprintf(buf.data(), buf.size(), "%ld px",
					                              (uint32_t)textdist);
		...
		/// Resize Boxes
		DrawBox(corner[0].x - 5, corner[0].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(corner[0].x - 4, corner[0].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
		DrawBox(corner[1].x - 5, corner[1].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(corner[1].x - 4, corner[1].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
		DrawBox(corner[2].x - 5, corner[2].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(corner[2].x - 4, corner[2].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
		DrawBox(corner[3].x - 5, corner[3].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(corner[3].x - 4, corner[3].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
		DrawBox(edge[0].x - 5, edge[0].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(edge[0].x - 4, edge[0].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
		DrawBox(edge[1].x - 5, edge[1].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(edge[1].x - 4, edge[1].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
		DrawBox(edge[2].x - 5, edge[2].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(edge[2].x - 4, edge[2].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
		DrawBox(edge[3].x - 5, edge[3].y - 5, 10, 10, 0, dp->m_resizeOuterColor,
		        dp->m_triangles);
		DrawBox(edge[3].x - 4, edge[3].y - 4, 8, 8, 0, dp->m_resizeInnerColor,
		        dp->m_triangles);
	}
...
void Display::DisplayCallback(void *_dp, uint32_t cx, uint32_t cy)
{
	...
	if (dp->m_source)
		...
		if (dp->m_shouldDrawUI) {
			...
			gs_ortho((float)-dp->m_previewOffset.first,
			         (float)(dp->m_gsInitData.cx - dp->m_previewOffset.first),
			         (float)-dp->m_previewOffset.second,
			         (float)(dp->m_gsInitData.cy - dp->m_previewOffset.second),
			         -100.0f, 100.0f);
...
Display::Display(uint64_t windowHandle,
                 obs_source_t *source) : Display(windowHandle)
{
